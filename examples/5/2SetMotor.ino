...
void SetMotor(int speed, int intDirection, String comment) {
    float directionRadians;
    float direction;
    float leftWheelFactor, rightWheelFactor;
    int leftWheelSpeed, rightWheelSpeed;
// printf("***** 99 SetMotor called\n\r");
...
if (glfSetMotorDebug) {
    // Serial.printf("%s\r\n", comment.c_str());
    // Serial.printf("[DEBUG] Speed: %d, Direction: %.2f, Left Wheel Speed: %d, Right Wheel Speed: %d\n\r",
    //   speed, direction,  leftWheelSpeed, rightWheelSpeed);
     }
...
// Serial.printf("SetMotor moving robot Left Wheel %d Right Wheel %d\n", leftWheelSpeed, rightWheelSpeed);

mappedLeftSpeed = map(leftWheelSpeed, -20, 20, -glf_SPEED_MAX, glf_SPEED_MAX);
mappedRightSpeed = map(rightWheelSpeed, -20, 20, -glf_SPEED_MAX, glf_SPEED_MAX);

// Serial.printf("***** Calling robotMotorSetSpeeds with Left:%d Right:%d\n\r", mappedLeftSpeed, mappedRightSpeed);

robotMotorSetSpeeds(mappedLeftSpeed, mappedRightSpeed);
// Serial.printf("***** robotMotorSetSpeeds returned\n\r");

    /* Log the command */
    datalogger(g_trace, speed, direction, leftWheelSpeed, rightWheelSpeed);

  //Serial.println("SetMotor done");
