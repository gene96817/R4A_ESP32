...
PCA9685 pca9685old;//Instantiate a PCA9685 object
...
void PCA9685_Setup(void)
{
  ...
  pca9685old.setupSingleDevice(Wire, PCA9685_ADDRESS);
  pca9685old.setToFrequency(FREQUENCY);
}
...
void Servo_1_Angle(float angle)
{
  ...
  pca9685old.setChannelPulseWidth(PCA9685_CHANNEL_0, int(angle));
}
...
void Servo_2_Angle(float angle)
{
  angle = constrain(angle, 0, 180);
  angle = map(angle, 0, 180, 102, 512);
  pca9685old.setChannelPulseWidth(PCA9685_CHANNEL_1, int(angle));
}
...
void Motor_Move(int m1_speed, int m2_speed, int m3_speed, int m4_speed)
{
  ...
  if (m1_speed >= 0)
  {
    pca9685old.setChannelPulseWidth(PIN_MOTOR_M1_IN1, m1_speed);
    pca9685old.setChannelPulseWidth(PIN_MOTOR_M1_IN2, 0);
  }
  else
  {
    m1_speed = -m1_speed;
    pca9685old.setChannelPulseWidth(PIN_MOTOR_M1_IN1, 0);
    pca9685old.setChannelPulseWidth(PIN_MOTOR_M1_IN2, m1_speed);
  }
  if (m2_speed >= 0)
  {
    pca9685old.setChannelPulseWidth(PIN_MOTOR_M2_IN1, m2_speed);
    pca9685old.setChannelPulseWidth(PIN_MOTOR_M2_IN2, 0);
  }
  else
  {
    m2_speed = -m2_speed;
    pca9685old.setChannelPulseWidth(PIN_MOTOR_M2_IN1, 0);
    pca9685old.setChannelPulseWidth(PIN_MOTOR_M2_IN2, m2_speed);
  }
  if (m3_speed >= 0)
  {
    pca9685old.setChannelPulseWidth(PIN_MOTOR_M3_IN1, m3_speed);
    pca9685old.setChannelPulseWidth(PIN_MOTOR_M3_IN2, 0);
  }
  else
  {
    m3_speed = -m3_speed;
    pca9685old.setChannelPulseWidth(PIN_MOTOR_M3_IN1, 0);
    pca9685old.setChannelPulseWidth(PIN_MOTOR_M3_IN2, m3_speed);
  }
  if (m4_speed >= 0)
  {
    pca9685old.setChannelPulseWidth(PIN_MOTOR_M4_IN1, m4_speed);
    pca9685old.setChannelPulseWidth(PIN_MOTOR_M4_IN2, 0);
  }
  else
  {
    m4_speed = -m4_speed;
    pca9685old.setChannelPulseWidth(PIN_MOTOR_M4_IN1, 0);
    pca9685old.setChannelPulseWidth(PIN_MOTOR_M4_IN2, m4_speed);
  }
}
...
void Buzzer_Setup(void)
{
  ...
  //************* Arduino ESP32 migration from 2.x to 3.0
  // ledcSetup(BUZZER_CHN, BUZZER_FREQUENCY, 10);
  // ledcAttachPin(PIN_BUZZER, BUZZER_CHN);
  // ledcAttach is new function for Arduino-ESP32 v3.0
...
{
  // TRACK_SENSOR.begin(PCF8574_SDA, PCF8574_SCL);  // Incorrect 
  TRACK_SENSOR.begin(PCF8574_ADDRESS);           // Correct
  // You might need to use Wire.begin(PCF8574_SDA, PCF8574_SCL) before this if the library requires it.
}
...
// Reverse bit order within the composite value:
sensorValue[3] = 
    ((sensorValue[3] & 0x01) << 2) | 
    ((sensorValue[3] & 0x02) << 0) | 
    ((sensorValue[3] & 0x04) >> 2);

// Reversed order for outputs:
sensorValue[2] = (sensorValue[3] & 0x01) << 2; //Now on the right - 1
sensorValue[1] = (sensorValue[3] & 0x02) << 0; //Now in the middle - 2
sensorValue[0] = (sensorValue[3] & 0x04) >> 2; //Now on the left - 4
